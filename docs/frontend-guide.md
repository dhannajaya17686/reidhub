...
.sidebar > nav > ul > li > a.nav-link:not(.active) {
  color: red;
}
...
/* ❌ Avoid excessive float-based layouts */
```
...
- 🖼️ Use WebP with PNG fallbacks for images
...
ReidHub follows WCAG 2.1 AA accessibility standards:
...
<div class="navbar">
  <div class="item"><a href="/dashboard">Dashboard</a></div>
</div>
```
...
<button aria-label="Toggle sidebar" data-sidebar-toggle>☰</button>

<!-- Use aria-hidden for decorative elements -->
<span class="icon-star" aria-hidden="true">★</span>

<!-- Use aria-current for active navigation -->
<a href="/dashboard" aria-current="page" class="nav-item--active">Dashboard</a>
...
class Modal {
  constructor() {
    this.modal = document.querySelector('[role="dialog"]');
    this.closeBtn = this.modal.querySelector('[aria-label="Close"]');
...
/* ✅ Good contrast */
.text-primary {
  color: #0F172A;           /* Dark on light background */
  background: #FFFFFF;
}

/* ✅ Good contrast for links */
a {
  color: #0466C8;           /* Blue links */
}

/* ❌ Poor contrast */
.text-weak {
  color: #A8B3B5;           /* Light gray on white */
  background: #FFFFFF;
}
```
...
<small id="pwd-hint">At least 8 characters, one uppercase letter</small>
...
ReidHub's frontend architecture emphasizes:
- **Modularity** - Feature-based organization mirroring backend structure
...
This structure enables developers to quickly understand the codebase, create new features consistently, and maintain high-quality user experiences across all ReidHub modules.
