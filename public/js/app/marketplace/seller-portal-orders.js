...
class OrdersManager {
  ...
  updateStatusLocal(orderIdStr, newStatus) {
    ...
    const row = Array.from(document.querySelectorAll('#orders-tbody .order-row'))
      .find(r => r.querySelector('.order-id')?.textContent?.includes(`#${id}`));
      ...
      if (newStatus === 'delivered' || newStatus === 'canceled') {
        ...
        if (actions) {
          actions.innerHTML = `
            ...
            <a href="/dashboard/marketplace/orders/${id}/chat" class="action-btn chat-btn">Chat</a>
          `;
        }
      }
    }
...
function manageOrder(orderId, status, paymentMethod) {
  ...
  window._ordersManager = window._ordersManager || new OrdersManager();
